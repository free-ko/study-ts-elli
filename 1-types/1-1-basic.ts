{
  ...
  age = undefined;
  age = 1;
  function find(): number | undefined {   // 데이터가 있거나 없을 때 undefined 많이 사용합니다.
    return undefined;
  }

  // null
  let person: null; // 💩
  let person2: string | null;   // 데이터 값이 있거나 없을 때 문맥상 null 사용하기도 함
  ...
  // void
  // 아무것도 리턴하지 않으면 void 타입이 됨
  function print(): void {
    console.log('hello');
    return;
  }

  // 변수에 void 타입을 설정할 경우 undefined 만 선언할 수 있어서 사용하지 않음
  let unusable: void = undefined; // 💩

  // never
  // 어플리케이션에서 예상치 못한 또는 핸들링 할 수 없는 에러가 발생하였을 때 호출 하는 함수 입니다.
	// 에러 메세지를 받아서 never 를 리턴하게 됩니다.
	// 그래서 발생한 에러 메시지를 서버로 보내 로그를 남기고 다시 어플리케이션에서 에러를 던질 수가 있습니다.
	// 에러를 던지고 어플리케이션이 죽게 됩니다.
	// 이 함수의 리턴 값을 naver 로 명시하게 되면 이 함수를 호출할 경우 리턴할 것이 없으니 그 점을 감안하고 코딩해라는 것과 동일합니다.
	// 또한 리턴되지 않는 경우 중
	// while 문을 true 로 지정해서 계속 실행되는 되는 상태가 되었을 때 리턴 값이 never 가 됩니다.
